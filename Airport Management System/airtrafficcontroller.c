...
int main(){
    ...
    // msgrcv(msgid, (void *)&planeRec, sizeof(struct planeInfo), -10,  0);
    //   printf("%d",planeRec.planeId);
    ...
    while(1){
        ...
        //printf("deep %d or arrive %d rz %d \n",dep,arv,rz);
    ...
    // if (msgrcv(msgid, (void *)&x, sizeof(struct planeInfo), 51,  0) != -1)
    //     {
    ...
    //     if (msgrcv(msgid, (void *)&planeRec, sizeof(struct planeInfo), -20, IPC_NOWAIT) != -1)
    // {
    ...
    // }
    // else if(planeRec.mtype){
    //     if(planeRec.mtype==planeRec.airportDearture+10){
    //         planeRec.mtype=planeRec.airPortArrival+10;
    ...
    //     }
    //     else{
    //         planeRec.mtype=planeRec.planeId;
    //         arv++;
    //         if (msgsnd(msgid, (void *)&planeRec, sizeof(struct planeInfo), 0) == -1)
    //         {
    //             // printf("error in sending message %s",buf);
